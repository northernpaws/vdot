...
void CollaborationSession::add_local_avatar(Avatar* p_avatar) {
    ERR_FAIL_COND_MSG(local_avatars.has(p_avatar), "Local avatar is already joined to the session.");
...
void CollaborationSession::remove_local_avatar(Avatar* p_avatar) {
    ERR_FAIL_COND_MSG(local_avatars.has(p_avatar), "Local avatar hasn't been added to the session.");
