...
godot::Error ModelBundle::add_directory( const godot::String &p_path,
                                         const godot::String &p_basepath, bool p_recursive ) {
    auto dir = godot::DirAccess::open( p_path );
    ERR_FAIL_COND_V_MSG(
        dir->get_open_error() != godot::OK, dir->get_open_error(),
        godot::vformat( "Failed to open directory to add to model bundle: %s", p_path ) );
    ...
    for ( const auto &filepath : dir_files ) {
        ...
        ERR_FAIL_COND_V_MSG( err != godot::OK, err,
                             godot::vformat( "Error adding file from directory to model bundle: %s",
    ...
    if ( p_recursive ) {
        ...
        for ( const auto &dirpath : dir_directories ) {
            ...
            ERR_FAIL_COND_V_MSG( err != godot::OK, err,
                                 godot::vformat( "Error adding directory to model bundle: %s",
...
godot::Error ModelBundle::add_file( const godot::String &p_path, const godot::String &p_basepath ) {
    ...
    ERR_FAIL_COND_V_MSG( file->get_open_error() != godot::OK, file->get_open_error(),
...
void ModelBundle::set_file_data( const godot::String &p_path,
                                 const godot::PackedByteArray &p_data ) {
    ERR_FAIL_COND_MSG( !files.has( p_path ),
...
godot::PackedByteArray ModelBundle::get_file_data( const godot::String &p_path ) const {
    ERR_FAIL_COND_V_MSG( !files.has( p_path ), {},
                         godot::vformat( "Specified file %s not found.", p_path ) );
...
godot::Error ModelBundle::unpack_bundle( const godot::String &p_path ) {
    ...
    ERR_FAIL_COND_V_MSG( dir_access->get_open_error() != godot::OK, dir_access->get_open_error(),
    ...
    for ( const godot::KeyValue<godot::String, BundleFile> &bundled_file : files ) {
        ...
        ERR_FAIL_COND_V_MSG( file_handle->get_open_error() != godot::OK,
                             file_handle->get_open_error(), "Failed to open file for writing." );
