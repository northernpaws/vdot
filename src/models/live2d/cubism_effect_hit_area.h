...
protected:
    static void _bind_methods() {
        godot::ClassDB::bind_method(godot::D_METHOD("set_target", "target"), &CubismEffectHitArea::set_target);
        godot::ClassDB::bind_method(godot::D_METHOD("get_target"), &CubismEffectHitArea::get_target);

        godot::ClassDB::bind_method(godot::D_METHOD("get_detail", "model", "id"), &CubismEffectHitArea::get_detail);

        godot::ClassDB::bind_method(godot::D_METHOD("set_monitoring", "value"), &CubismEffectHitArea::set_monitoring);
        godot::ClassDB::bind_method(godot::D_METHOD("get_monitoring"), &CubismEffectHitArea::get_monitoring);
        ADD_PROPERTY(godot::PropertyInfo(godot::Variant::BOOL, "monitoring"), "set_monitoring", "get_monitoring");

        ADD_SIGNAL(godot::MethodInfo(
            SIGNAL_EFFECT_HIT_AREA_ENTERED,
            godot::PropertyInfo(godot::Variant::OBJECT, "model", godot::PROPERTY_HINT_RESOURCE_TYPE, "GDCubismUserModel"),
...
private:
    ...
    godot::Rect2 build_rect2(const godot::Ref<godot::ArrayMesh>& ref_ary_mesh) const {
    ...
    godot::Array check_collide_triangle(const godot::Ref<godot::ArrayMesh>& ref_ary_mesh) const {
        if(ref_ary_mesh->surface_get_primitive_type(0) != godot::Mesh::PRIMITIVE_TRIANGLES) return godot::Array();
...
public:
    void set_target(const godot::Vector2& target) { this->_target = target; this->_target_update = true; }
    ...
    godot::Dictionary get_detail(const CubismModel* model, const godot::String& id) {
        if(this->_initialized == false) return godot::Dictionary();
        if(model == nullptr) return godot::Dictionary();
        if(model->is_initialized() == false) return godot::Dictionary();

        const godot::Dictionary dict_mesh = model->get_meshes();
        if(dict_mesh.is_empty() == true) return godot::Dictionary();

        godot::Ref<godot::ArrayMesh> ref_ary_mesh = dict_mesh[id];
        if(ref_ary_mesh.is_null() == true) return godot::Dictionary();
        if(ref_ary_mesh->surface_get_array_index_len(0) < 3) return godot::Dictionary();

        godot::Dictionary dict_result;

        godot::Rect2 rect = this->build_rect2(ref_ary_mesh);
    ...
    virtual void _cubism_init(CubismModelProxy* internal_model) override {
    ...
    virtual void _cubism_term(CubismModelProxy* internal_model) override {
    ...
    virtual void _cubism_process(CubismModelProxy* model, const float delta) override {
        ...
        for(int64_t i = 0; i < ary.size(); i++) {
            const godot::String id = static_cast<godot::Dictionary>(ary[i]).get("id", godot::String());
            if(dict_mesh.has(id) != true) continue;

            godot::Ref<godot::ArrayMesh> ref_ary_mesh = dict_mesh[id];
            if(ref_ary_mesh->surface_get_array_index_len(0) < 3) continue;

            godot::Rect2 check_rect = this->build_rect2(ref_ary_mesh);
