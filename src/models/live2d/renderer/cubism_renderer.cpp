    ...
    , adjust_scale(1.0)
{
    ...
    this->ary_shader[CUBISM_SHADER_NORM_ADD] = res_loader->load("res://addons/vdot/live2d/shader/2d_cubism_norm_add.gdshader");
    this->ary_shader[CUBISM_SHADER_NORM_MIX] = res_loader->load("res://addons/vdot/live2d/shader/2d_cubism_norm_mix.gdshader");
    this->ary_shader[CUBISM_SHADER_NORM_MUL] = res_loader->load("res://addons/vdot/live2d/shader/2d_cubism_norm_mul.gdshader");

    this->ary_shader[CUBISM_SHADER_MASK] = res_loader->load("res://addons/vdot/live2d/shader/2d_cubism_mask.gdshader");

    this->ary_shader[CUBISM_SHADER_MASK_ADD] = res_loader->load("res://addons/vdot/live2d/shader/2d_cubism_mask_add.gdshader");
    this->ary_shader[CUBISM_SHADER_MASK_ADD_INV] = res_loader->load("res://addons/vdot/live2d/shader/2d_cubism_mask_add_inv.gdshader");
    this->ary_shader[CUBISM_SHADER_MASK_MIX] = res_loader->load("res://addons/vdot/live2d/shader/2d_cubism_mask_mix.gdshader");
    this->ary_shader[CUBISM_SHADER_MASK_MIX_INV] = res_loader->load("res://addons/vdot/live2d/shader/2d_cubism_mask_mix_inv.gdshader");
    this->ary_shader[CUBISM_SHADER_MASK_MUL] = res_loader->load("res://addons/vdot/live2d/shader/2d_cubism_mask_mul.gdshader");
    this->ary_shader[CUBISM_SHADER_MASK_MUL_INV] = res_loader->load("res://addons/vdot/live2d/shader/2d_cubism_mask_mul_inv.gdshader");
...
void CubismRenderer::pro_proc(const Csm::csmInt32 viewport_count, const Csm::csmInt32 mesh_instance_count) {
...
void _recurisive_dispose_node(const godot::Node* parent_node, const bool node_release) {
    ...
    for(Csm::csmInt32 i = 0; i < ary_node.size(); i++) {
        ...
        if(godot::Object::cast_to<CubismEffect>(ary_node[i]) != nullptr) continue;

        godot::MeshInstance2D *m_node = godot::Object::cast_to<godot::MeshInstance2D>(ary_node[i]);
