...
bool CubismModelProxy::model_load(const godot::String &model_pathname) {
    ...
    this->_model_setting = CSM_NEW Live2D::Cubism::Framework::CubismModelSettingJson(buffer.ptr(), buffer.size());

    // setup Live2D model
    if (strcmp(this->_model_setting->GetModelFileName(), "") == 0) {
        return false;
    } else {
        godot::String gd_filename; gd_filename.parse_utf8(this->_model_setting->GetModelFileName());
        ...
        this->_moc3_file_format_version = static_cast<CubismModel::moc3FileFormatVersion>(version);
    }
...
void CubismModelProxy::model_load_resource()
{
    ...
    for (Live2D::Cubism::Framework::csmInt32 index = 0; index < this->_model_setting->GetTextureCount(); index++)
    {
        if (strcmp(this->_model_setting->GetTextureFileName(index), "") == 0) continue;

        godot::String gd_filename; gd_filename.parse_utf8(this->_model_setting->GetTextureFileName(index));
        godot::String texture_pathname = this->_model_pathname.get_base_dir().path_join(gd_filename);
...
void CubismModelProxy::clear() {
...

    {
        ...
        for(Live2D::Cubism::Framework::csmMap<Live2D::Cubism::Framework::csmString, Live2D::Cubism::Framework::CubismExpressionMotion*>::const_iterator i = this->_map_expression.Begin(); i != this->_map_expression.End(); i++) {
    ...
    {
        this->motion_stop();
        for(Live2D::Cubism::Framework::csmMap<Live2D::Cubism::Framework::csmString, Live2D::Cubism::Framework::CubismMotion*>::const_iterator i = this->_map_motion.Begin(); i != this->_map_motion.End(); i++) {
            Live2D::Cubism::Framework::ACubismMotion::Delete(i->Second);
        }
...
void CubismModelProxy::expression_set(const char* expression_id) {
    ...
    Live2D::Cubism::Framework::ACubismMotion* motion = this->_map_expression[Live2D::Cubism::Framework::csmString(expression_id)];
...
Live2D::Cubism::Framework::CubismMotionQueueEntryHandle CubismModelProxy::motion_start(const char* group, const int32_t no, const int32_t priority, const bool loop, const bool loop_fade_in, void* custom_data) {
    ...
    Live2D::Cubism::Framework::csmString name = Live2D::Cubism::Framework::Utils::CubismString::GetFormatedString("%s_%d", group, no);

    Live2D::Cubism::Framework::CubismMotion* motion = this->_map_motion[name];

    if(motion == nullptr ) return Live2D::Cubism::Framework::InvalidMotionQueueEntryHandleValue;
...
void CubismModelProxy::MotionEventFired(const Live2D::Cubism::Framework::csmString& eventValue) {
    if(this->_owner_viewport != nullptr) {
        godot::String value; value.parse_utf8(eventValue.GetRawString());
...
void CubismModelProxy::expression_load() {
    ...
    for (Live2D::Cubism::Framework::csmInt32 i = 0; i < this->_model_setting->GetExpressionCount(); i++)
    {
        Live2D::Cubism::Framework::csmString name = this->_model_setting->GetExpressionName(i);

        godot::String gd_filename; gd_filename.parse_utf8(this->_model_setting->GetExpressionFileName(i));
        godot::String expression_pathname = this->_model_pathname.get_base_dir().path_join(gd_filename);

        godot::PackedByteArray buffer = godot::FileAccess::get_file_as_bytes(expression_pathname);
        Live2D::Cubism::Framework::CubismExpressionMotion* motion = static_cast<Live2D::Cubism::Framework::CubismExpressionMotion*>(this->LoadExpression(
            buffer.ptr(),
            buffer.size(),
...
void CubismModelProxy::physics_load() {
    ...
    godot::String gd_filename; gd_filename.parse_utf8(this->_model_setting->GetPhysicsFileName());
...
void CubismModelProxy::pose_load() {
    ...
    godot::String gd_filename; gd_filename.parse_utf8(this->_model_setting->GetPoseFileName());
...
void CubismModelProxy::userdata_load() {
    ...
    godot::String gd_filename; gd_filename.parse_utf8(this->_model_setting->GetUserDataFile());
...
void CubismModelProxy::motion_load() {
    ...
    for (Live2D::Cubism::Framework::csmInt32 ig = 0; ig < this->_model_setting->GetMotionGroupCount(); ig++)
    {
        //PreloadMotionGroup(group);
        const Live2D::Cubism::Framework::csmChar* group = this->_model_setting->GetMotionGroupName(ig);
        const Live2D::Cubism::Framework::csmInt32 motion_count = this->_model_setting->GetMotionCount(group);

        if(motion_count == 0) continue;

        for (Live2D::Cubism::Framework::csmInt32 im = 0; im < motion_count; im++)
        {
            Live2D::Cubism::Framework::csmString name = Live2D::Cubism::Framework::Utils::CubismString::GetFormatedString("%s_%d", group, im);

            godot::String gd_filename; gd_filename.parse_utf8(this->_model_setting->GetMotionFileName(group, im));
            godot::String motion_pathname = this->_model_pathname.get_base_dir().path_join(gd_filename);

            godot::PackedByteArray buffer = godot::FileAccess::get_file_as_bytes(motion_pathname);
            Live2D::Cubism::Framework::CubismMotion* motion = static_cast<Live2D::Cubism::Framework::CubismMotion*>(this->LoadMotion(
                buffer.ptr(),
                buffer.size(),
                name.GetRawString()
                    ));

            Live2D::Cubism::Framework::csmFloat32 fade_time_sec = this->_model_setting->GetMotionFadeInTimeValue(group, im);
            ...
            static_cast<Live2D::Cubism::Framework::CubismMotion*>(motion)->SetEffectIds(this->_list_eye_blink, this->_list_lipsync);
...
void CubismModelProxy::effect_init() {
    for(
        Csm::csmVector<CubismEffect*>::iterator i = this->_owner_viewport->_list_cubism_effect.Begin();
