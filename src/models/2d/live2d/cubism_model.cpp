...
void CubismModel::set_assets( const godot::String p_assets ) {
    ...
    if ( f.is_null() ) {
        this->clear();
        ERR_FAIL_MSG( godot::vformat( "Failed to open .model3.json file '%s': %s", p_assets,
                                      f->get_open_error() ) );
    }
    ...
    if ( !this->proxy_model->model_load( p_assets ) ) {
        this->clear();
        ERR_FAIL_MSG(
            godot::vformat( "Failed to load model from .model3.json file '%s'", p_assets ) );
    }
    if ( !this->proxy_model->IsInitialized() ) {
        this->clear();
        ERR_FAIL_MSG(
            godot::vformat( "Failed to initialize model from .model3.json file '%s'", p_assets ) );
    }

    {
        ...
        if ( strcmp( this->proxy_model->_model_setting->GetDisplayInfoFileName(), "" ) != 0 ) {
            ...
            auto displayinfo_file =
                godot::FileAccess::open( p_assets.get_base_dir().path_join( display_info_filename ),
                                         godot::FileAccess::ModeFlags::READ );
            ERR_FAIL_COND_MSG(
                displayinfo_file->get_open_error(),
                godot::vformat( "Failed to open display info file: ",
        ...
        for ( Csm::csmInt32 index = 0; index < model->GetParameterCount(); index++ ) {
            auto param_id =
                godot::String( Live2D::Cubism::Core::csmGetParameterIds( csm_model )[index] );

            // TODO: do we need to expose Live2D::Cubism::Core::csmGetParameterTypes( csm_model
            // )[index];
