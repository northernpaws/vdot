...
godot::Error Live2DModelBundle::pack_from_model3(const godot::String& p_path) {
    ...
    ERR_FAIL_COND_V_MSG(buffer.size() == 0, godot::FAILED, godot::vformat("Failed to read file: %s", p_path));
    ...
    if ( strcmp( model_setting->GetUserDataFile(), "" ) != 0 ) {
        ...
        err = add_file(p_path.get_base_dir().path_join(userdata_filename), p_path.get_base_dir());
    ...
    for ( int i = 0; i < model_setting->GetTextureCount(); i++) {
        ...
        ERR_FAIL_COND_V_MSG(err != godot::OK, err, godot::vformat("Failed to pack texture: %s", tex_filename));
    }
    ...
    for ( int i = 0; i < model_setting->GetExpressionCount(); i++) {
        ...
        ERR_FAIL_COND_V_MSG(err != godot::OK, err, godot::vformat("Failed to pack expression: %s", exp_filename));
    }
    ...
    for ( int i = 0; i < model_setting->GetMotionGroupCount(); i++) {
        ...
        for ( int x = 0; x < model_setting->GetMotionCount(group_name); x++) {
            ...
            err = add_file( p_path.get_base_dir().path_join( motion_filename ), p_path.get_base_dir() );
