...
void CubismEffectFaceTracking::_bind_methods() {
    godot::ClassDB::bind_method( godot::D_METHOD( "_server_tracker_added", "tracker_name", "tracker_type" ),
...
bool CubismEffectFaceTracking::_set(const godot::StringName &p_name, const godot::Variant &p_property) {
    if (p_name == godot::StringName("tracker")) {
        TrackingServer* server = TrackingServer::get_singleton();
        ERR_FAIL_COND_V_MSG(server == nullptr, false, "Expected tracking server to be initialized.");
...
bool CubismEffectFaceTracking::_get(const godot::StringName &p_name, godot::Variant &r_property) const {
    if (p_name == godot::StringName("tracker")) {
        if (_tracker.is_valid()) {
...
void CubismEffectFaceTracking::_get_property_list(godot::List<godot::PropertyInfo> *p_list) const {
    TrackingServer* server = TrackingServer::get_singleton();
    ERR_FAIL_COND_MSG(server == nullptr, "Expected tracking server to be initialized.");
...
void CubismEffectFaceTracking::_server_tracker_added(const godot::StringName& p_tracker_name, TrackingServer::TrackerType p_tracker_type) {
